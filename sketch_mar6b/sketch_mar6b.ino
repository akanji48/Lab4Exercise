               ...
               if(bt_Motors_Enabled)                                          // Run motors only if enabled
               {
                  if(bt_2_S_Time_Up)                                          // Update drive state after 2 seconds
                  {
                     ...
                     switch(uc_Drive_Index)                                   // Cycle through drive states
                     {
                        ...
                        case 1: // Drive forward WORKS
                        {
                           Bot.Forward("D1", map(2000, 0, 4096, 150, 255), map(2000, 0, 4096, 150, 255)); // Drive ID, Left speed, Right speed
                           if(!digitalRead(LIMIT_SWITCH)){//if switch touched, goes to next case
                             uc_Drive_Index = 2;
                             Bot.Stop("D1");
                             }
                           break;
                        ...
                        case 3: // bring shoulder back WORKS
                        {
                           Bot.ToPosition("S2", map(4300, 0, 4096, ci_Shoulder_Servo_Retracted,  ci_Shoulder_Servo_Extended));  //shoulder back
                           Bot.ToPosition("S1", map(0, 0, 4096, ci_Claw_Servo_Open, ci_Claw_Servo_Closed));                //claw open
                        ...
                        {
                          Bot.Stop("D1");
                          Bot.ToPosition("S2", map(6000, 0, 4096, ci_Shoulder_Servo_Retracted,  ci_Shoulder_Servo_Extended));//shoulder forward
                          uc_Drive_Index = 6;
                          break;
                        }
                        case 6:
                        ...
                        case 7:
                        {
                          Bot.ToPosition("S2", map(4000, 0, 4096, ci_Shoulder_Servo_Retracted,  ci_Shoulder_Servo_Extended));//lifts
                          uc_Drive_Index = 8;
                          //shoulder back
                          break;
                        }
                        case 8:
                        {
                          Bot.Reverse("D1", map(2000, 0, 4096, 150, 255));//drives back
                          uc_Drive_Index = 9;
                          break;
                        }
                        case 9:
                        ...
                        case 11:
                        {
                          Bot.ToPosition("S1", map(0, 0, 4096, ci_Claw_Servo_Open, ci_Claw_Servo_Closed)); //open claw
                          break;
                        }
